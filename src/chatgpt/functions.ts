...
X-axis shows time. To calculate the points, use the pivot points provided by the pivot_points tool function
being within the time range indicated by the user.
Calculate at least 50 points of the BTC/USD price for the time range indicated by the user, 
with the time interval between the points being equal.
Draw the line going trough the calculated points.
Please provide the exact coordinates of the points of the line in the description, 
using number of the point for X axis and the price for Y axis,
with all time intervals between the points being the same.
The chart should be simple - only line, X axis, and Y axis. Do not include any other elements.
Put the BTC/USD maximum price on the top of the Y axis and the minimum BTC/USD price on the bottom of the Y axis.
Do not put anything on the X axis.
Put the period for which the chart is drawn on the top of the chart (for example 1 day, 1 week, 1 month).
Do not put any other text on the chart. Do not put any other elements on the chart, 
except for the line going thru the points, X axis, and Y axis, and the period for which the chart is drawn.
Theme: professional dark.
Colors: green, red.
Provide the maximum detailed description of the BTC/USD chart image to be generated by DALL-E-3.
